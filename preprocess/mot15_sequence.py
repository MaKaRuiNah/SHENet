...
class MOT15Sequence(Dataset):
    ...
    def sequence(self, seq_name):
            ...
            with open(gt_file, "r") as inf:
                ...
                for row in reader:
                    ...
                    if int(row[6]) == 1: #and float(row[8]) >= self.vis_threshold:
                        ...
                        bb = np.array([(x1+x2)/2,(y1+y2)/2], dtype=np.float32)
        ...
        for i in range(1,seq_length+1):
            ...
            total.append(sample)
        # split_num = int(0.8*seq_length)
        # return total[split_num:], no_gt
        return total, no_gt
import pickle
if __name__ == '__main__':
    ...
    path = "./data/2DMOT2015/train/Venice-2.pkl"
